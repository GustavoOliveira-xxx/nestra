...
import { sql, environmentToClient, itemToClient, prefsToClient } from '../_lib/db.js';
...
export default handler(async (req, res) => {
  ...
  const [envs, items, prefs] = await Promise.all([
    sql`select * from environments where owner_id = ${user.id} and archived_at is null order by position`,
    sql`
      select i.*, coalesce(
